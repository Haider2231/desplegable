...
export default function Header() {
  ...
  // Al abrir el menú de perfil, consulta si mostrar el botón de establecimientos
  useEffect(() => {
    async function checkEstablecimientos() {
      if (!user) return setMostrarMisEstablecimientos(false);
      try {
        const res = await fetch(`/establecimientos/dueno/${user.userId}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
        });
        const data = await res.json();
        if (Array.isArray(data) && data.some(e => e.estado === "pendiente" || e.estado === "rechazado")) {
          setMostrarMisEstablecimientos(true);
        } else {
          setMostrarMisEstablecimientos(false);
        }
      } catch {
        setMostrarMisEstablecimientos(false);
      }
    }
    if (showProfileMenu) checkEstablecimientos();
                    ...
                    {user?.rol === "admin" ? (
                      ...
                      <>
                        ...
                        {user?.rol === "propietario" ? (
                          <button
                            style={profileMenuBtnStyle}
                            onClick={() => { setShowProfileMenu(false); navigate("/historial-abonos-propietario"); }}
                          >
                            🧾 Historial de abonos 
                          </button>
                        ) : (
                          <button
                            style={profileMenuBtnStyle}
                            onClick={() => { setShowProfileMenu(false); navigate("/historial-abonos"); }}
                          >
                            🧾 Historial de abonos
                          </button>
                        )}
                        {user?.rol && mostrarMisEstablecimientos && (
                          <button
                            style={profileMenuBtnStyle}
                            onClick={() => { setShowProfileMenu(false); navigate("/mis-establecimientos"); }}
                          >
